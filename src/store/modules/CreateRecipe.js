...
const createRecipe = {
    ...
    actions: {
      ...
      //Main
      async CreateMainIngredient({ commit, getters }, mIngredients) {
        const recipeID = getters.findThisRecipeID;
        console.log("recipeID M ",recipeID);   
        await axios        
          .post(`${process.env.VUE_APP_BACKEND}/api/ingredient/createRecipeIngredient/${recipeID}`,mIngredients)
          .then((response) => {
      ...
      //Sub
      async CreateSubIngredient({ commit,getters }, sIngredients) {
        const recipeID = getters.findThisRecipeID;
        console.log("recipeID S ",recipeID);   
        await axios
          .post(`${process.env.VUE_APP_BACKEND}/api/ingredient/createRecipeIngredient/${recipeID}`,sIngredients)
          .then((response) => {
      ...
      async CreateFlavoring({ commit,getters }, flavoring) {
        ...
        await axios
          .post(`${process.env.VUE_APP_BACKEND}/api/ingredient/createRecipeIngredient/${recipeID}`,flavoring)
          .then((response) => {
